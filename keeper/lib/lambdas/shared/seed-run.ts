...
// =============================================================================
//
// Devnet only. Keeps every named market funded on both sides and claims what
...
//
// The wallets are derived from the publisher key by index. Which indices a
// run uses is the caller's decision: equities and crypto draw from disjoint
// ranges, so one venue running its allowance down can never leave the
// other's markets one-sided.
//
// Side and amount are random on each first deposit, with one rule: an empty
// side is always filled first, so no market is left one-sided by the draw.
//
// Idempotent per wallet, market and side. A wallet that already holds a
// position on a market is skipped, so a retried run never deposits twice and
// real deposits are never displaced.
//
...
import { LIVE_CUTOFF_SECS } from "./config";
import {
...
export interface SeedRunOptions {
  /** Which markets to fund and claim. Ones that do not exist are skipped. */
  specs: readonly SeedSpec[];
  /** Derivation indices of the wallets this run may spend. */
  walletIndices: readonly number[];
  logger: Logger;
}
...
/** Funds both sides of every existing market in `specs` and claims what the wallets have won. */
export async function runSeeder(
  options: SeedRunOptions,
): Promise<{ deposited: string[]; claimed: string[] }> {
  const { specs, walletIndices, logger } = options;
  ...
  for (const index of walletIndices) {
    const keypair = Keypair.fromSeed(seedBytes(publisher.secretKey, index));
    wallets.push({ index, keypair, ata: getAssociatedTokenAddressSync(mint, keypair.publicKey) });
  }
  logger.info("seed wallets", {
    indices: [...walletIndices],
    addresses: wallets.map((w) => w.keypair.publicKey.toBase58()),
  });
