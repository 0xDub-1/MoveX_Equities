...
// =============================================================================
//
// The keeper-published PriceFeed account of every listed asset, polled on a
// short cadence and additionally subscribed over the RPC websocket so a fresh
// print lands in the UI the moment it lands on chain.

import { useEffect } from "react";
import { useQuery, useQueryClient } from "@tanstack/react-query";
import { useConnection } from "@solana/wallet-adapter-react";

import { ALL_SYMBOLS } from "@/lib/assets";
import { MAX_PRICE_AGE_SECS, POLL_MS } from "@/lib/config";
...
const FEED_ADDRESSES = ALL_SYMBOLS.map((symbol) => ({ symbol, address: priceFeedPda(symbol) }));
